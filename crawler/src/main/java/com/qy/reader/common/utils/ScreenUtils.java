...
public class ScreenUtils {
    ...
    public static int getScreenHeight() {
        int resourceId = BaseLibApp.Companion.getInstance().getResources()
                .getIdentifier("notch_height", "dimen", "android");
        int result = 0;
        if (resourceId > 0) {
            result = BaseLibApp.Companion.getInstance().getResources().getDimensionPixelSize(resourceId);
        }
        return com.blankj.utilcode.util.ScreenUtils.getScreenHeight() - result;
    }
