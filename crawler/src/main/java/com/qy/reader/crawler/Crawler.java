...
import com.qy.reader.common.utils.ListUtil;
        ...
        for (int i = 0; i < SourceManager.CONFIGS.size(); i++) {
            ...
            try { // 提高容错性
                for (JXNode jxNode : rs) {
                    ...
                    String link;
                    if (source.id == SourceID.CANGSHU99) {
//                        99藏书
                        link = " https://m.99lib.net/book/" + getNodeStr(jxNode, config.search.linkXpath) + "/index.html";
        ...
        try {
            for (JXNode jxNode : rs) {
                ...
                if (!TextUtils.isEmpty(link)) {
                    if (source.id == 22) {
//                        99藏书，单独处理
                        chapter.link = "https://m.99lib.net" + link;
                    } else {
    ...
    public static void content(SearchBook.SL sl, String url, ContentCallback callback) {
        ...
        try {
            ...
            String content;
            if (source.id == 22) {
                content = getNodeStr99(jxDocument, config.content.xpath);
            } else {
                content = getNodeStr(jxDocument, config.content.xpath);
            }
    ...
    //99藏书的内容文字需要特殊的处理
    private static String getNodeStr99(Object startNode, String xpath) {
